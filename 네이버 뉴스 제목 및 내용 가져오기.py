...
for i in range(count):                      # 해당 개수만큼 반복
    pos1 = source_data.find('"cjs_t">')+ len('"cjs_d">')     # 뉴스 앞 부분위치 지정
    source_data = source_data[pos1:]                         # 해당 위치로 이동

    pos2 = source_data.find('</p>')       # 뉴스 뒷부분 위치 지정
    extract_data = source_data[: pos2]      # 앞부분으로부터 뒷부분까지 내용추출해서 저장
